...
<p align="center">
<img alt="Swift Version" src="https://img.shields.io/badge/6.2-red?style=flat&logo=Swift&logoColor=white&label=Swift&labelColor=light_gray&color=orange"/>
...
Setting up a local server basically does not require any changes to the application source code, so it works almost the same way as the release version of the application. The defined server responses are in the same form as the responses of the real server. This ensures that the tests are able to check the correctness of decoding, mapping data, UI and etc.
